...
- **플랫폼 및 아키텍처 고도화**  
  ...
  -  **Orchestrator 패턴을 활용**하여 서비스 계층에서 벡터 검색, OpenAI 기반 챗봇 답변 생성, 사용 토큰 저장, 그리고 답변 로그 저장 흐름을 통합적으로 캡슐화
...
- 마이그레이션으로 기존 동기적 구조는 모든 단계가 순차적으로 수행되어 외부 API 지연 시 전체 응답이 지연되는 한계. 
    → **비동기/병렬 처리 구조를 도입**해 독립된 I/O 작업을 분리 실행함으로써 응답 속도와 리소스 효율을 개선 필요.
---
...
- Embedding, Vector Search 등 I/O 병목은CompletableFuture 에서 점진적 Mono (Spring WebFlux)로 병렬화, 통계/로그/알림 같은 비동기 후처리는 Kafka/Redis 이벤트 큐 기반으로 처리하여 확장성과 응답 성능을 모두 확보.
---
