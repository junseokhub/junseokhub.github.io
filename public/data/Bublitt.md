...
### 🛠️ **대안 및 향후 방향**
- eps, min_samples
  - **HDBSCAN** 도입 검토. → 계층적 클러스터링을 통해 eps 값을 명시적으로 지정하지 않아도 클러스터를 효과적으로 식별하며, 밀도가 다른 클러스터도 유연하게 처리 필요.
  - **UMAP** 적용 고려. → 차원 축소 기법 적용을 고려. 이는 시각화를 통해 클러스터의 특성을 이해하고, 클러스터링 전 노이즈 제거 및 데이터 정제에 도움 예상.
